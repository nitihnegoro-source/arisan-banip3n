...
export const CashInView: React.FC<CashInViewProps> = ({
  ...
  const otherInTxs = inTransactions.filter(
    (t) => t.category !== 'Setoran Arisan' && t.category !== 'Setoran Iuran'
  );
  const totalOtherIn = otherInTxs.reduce((sum, t) => sum + t.amount, 0);
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      setSyncFeedback('Data setoran berhasil disinkronisasi ke Buku Kas Masuk!');
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 rounded-2xl bg-gradient-to-r from-teal-700 via-emerald-800 to-slate-900 p-6 text-white shadow-md">
        <div className="space-y-1.5">
          ...
          <p className="text-xs sm:text-sm text-teal-100/90 max-w-2xl">
            Semua penerimaan kas masuk terhubung langsung dan otomatis dengan pembayaran <strong>Setoran Arisan (Rp 50.000)</strong> dan <strong>Setoran Iuran (Rp 20.000)</strong> serta donasi/saldo awal.
          </p>
        </div>
          ...
          <div className="flex flex-col gap-2">
            {onSyncFinance && (
              <button
                ...
                title="Sinkronkan ulang seluruh data setoran anggota"
              >
                <RefreshCw className={`h-3.5 w-3.5 ${isSyncing ? 'animate-spin' : ''}`} />
                <span>{isSyncing ? 'Menyinkronkan...' : 'Sinkronkan Setoran'}</span>
              </button>
            )}
            <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        >
          ...
          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
            Tersinkron otomatis saat anggota bayar arisan
          </p>
        </div>
        ...
        <div 
          ...
          className={`rounded-2xl p-4 border transition-all cursor-pointer ${
            categoryFilter === 'Setoran Iuran' 
              ? 'bg-teal-50 dark:bg-teal-950/30 border-teal-400 ring-2 ring-teal-400/30' 
        ...
        >
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-teal-100 dark:bg-teal-900/50 text-teal-700 dark:text-teal-300">
          ...
          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
            Tersinkron otomatis saat anggota bayar iuran
          </p>
        </div>

        {/* Penerimaan Lainnya Card */}
        <div 
        ...
        >
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300">
              {otherInTxs.length} Transaksi
            </span>
          </div>
          <p className="text-xl font-extrabold text-slate-900 dark:text-white">
            {formatRupiah(totalOtherIn)}
          </p>
          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
            Dicatat manual melalui tombol Catat Kas Masuk
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 font-bold uppercase tracking-wider">
                ...
                <th className="py-3.5 px-4">Sumber / Penyetor</th>
